...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    if (!manualOverride) label = randomLabel();
    applyLabel(ctx);
  });
  ...
  pi.registerCommand("thinking-label", {
    description: "Set the hidden thinking label. Use without args to reset to random mode.",
    handler: async (args, ctx) => {
      ...
      if (!nextLabel) {
        ...
        label = randomLabel();
        ctx.ui.setHiddenThinkingLabel(label);
        ctx.ui.notify(`Hidden thinking label reset to random mode: ${label}`);
        return;
      }
